#!/usr/bin/env python3
"""
Complete Enhanced Web-based Local AI Assistant - FINAL FIXED VERSION
    ...
    def _create_default_config(self):
        ...
        class DefaultConfig:
            def __init__(self):
                ...
                self.data = {"users": {}, "configured": False}
    ...
    <div class="auth-container">
        ...
        <form id="signupForm" class="auth-form">
            <div class="form-group">
                ...
                <input type="text" id="signupUsername" class="form-input" required minlength="3" maxlength="20">
            </div>
            <div class="form-group">
...
<body>
    ...
    def handle_signin(self):
        ...
        try:
            ...
            if not username or not password:
                self.send_json_response({"success": False, "message": "Username and password are required"})
                return
            ...
            self.send_json_response({"success": False, "message": "Login failed - please try again"})
    ...
    def handle_signup(self):
        ...
        try:
            ...
            if not username or len(username) < 3:
                self.send_json_response({"success": False, "message": "Username must be at least 3 characters long"})
                return
            
            if not email or '@' not in email:
                self.send_json_response({"success": False, "message": "Please enter a valid email address"})
                return
            
            if not password or len(password) < 6:
                self.send_json_response({"success": False, "message": "Password must be at least 6 characters long"})
                return
            ...
            # Create user - FIXED: Using correct method signature with email parameter
            password_hash = hashlib.sha256(password.encode()).hexdigest()
            self.config.add_user(username, password_hash, role, email)
            self.config.mark_configured()
            
            print(f"✅ User created successfully: {username} ({role}) - {email}")
            self.send_json_response({"success": True, "message": "Account created successfully"})
            
        except Exception as e:
            print(f"❌ Signup error: {e}")
            self.send_json_response({"success": False, "message": f"Failed to create account: {str(e)}"})
    ...
    def handle_chat(self):
        ...
        try:
            ...
            if not message:
                self.send_json_response({"success": False, "message": "Message cannot be empty"})
                return
...
def start_server(port=8080):
    ...
    try:
        ...
        if "Address already in use" in str(e):
            ...
            print(f"   python complete_web_assistant_final.py")
        else:
